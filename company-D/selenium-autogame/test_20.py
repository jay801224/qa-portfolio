...
class test_20gamespin(unittest.TestCase):
    ...
    def test2000(self):
        ...
        assert newcurrenturl == get_fruitgameurl('2000'), "当前遊戲网址非预期！"
        ...
        self.driver.save_screenshot('./result/'+get_day()+'\\'+get_time()+'2000-spin.png')
        time.sleep(3)
        #打開投注記錄頁面
        self.driver.execute_script("window.open()")
        ...
        for i in range(60):
            try:
                ...
                self.fail("time out")
        try: 
            self.assertEqual("Anti Narkoba", driver.find_element_by_xpath("//table[@id='__BVID__11']/tbody/tr/td[3]").text)
        except AssertionError as e: 
            self.verificationErrors.append(str(e))
    ...
    def test2001(self):
        ...
        urltoken = tokenpara.split('&betrecordurl')[0]
        print("跳轉網址前半段是：", newcurrenturl)
        assert newcurrenturl == get_fruitgameurl('2001'), "当前遊戲网址非预期！"
        pic_path = './result/'+get_day()+'\\'
        ...
        # work on firefox
        #ActionChains(driver).move_by_offset(-1075, -900).perform() # 将鼠标位置恢复到移动前 
        self.driver.save_screenshot('./result/'+get_day()+'\\'+get_time()+'2001-spin.png')
        time.sleep(3)
        #打開投注記錄頁面
        self.driver.execute_script("window.open()")
        self.driver.switch_to.window(driver.window_handles[2])
        ...
        for i in range(60):
            try:
                ...
                self.fail("time out")
        try: 
            self.assertEqual("Mainan 90'an", driver.find_element_by_xpath("//table[@id='__BVID__11']/tbody/tr/td[3]").text)
        except AssertionError as e: 
            self.verificationErrors.append(str(e))
...
if __name__ == "__main__":
    ...
    unittest.addTest(test_20gamespin("test2000")) 
    unittest.addTest(test_20gamespin("test2001"))
    #unittest.addTest(test_20gamespin("test2101"))
    #unittest.addTest(test_20gamespin("test2102"))

    #将测试结果写入到result.html中
    fp=open(get_time()+"-20-result.html",'wb')
    runner=HTMLTestRunner(stream=fp,title='20 Test Report',description='Result:',tester='jay')
    runner.run(unittest)
    fp.close()